...
class GPT2Generator:
    ...
    def tokenize(self, cxt):
        ...
        # 추가
        turns = turns[-min(max_turn, len(turns)):]
        ids = []
        for turn in turns:
...
class Integrated:
    ...
    def predict(self, cxt, wt_ranker, params):
        with torch.no_grad():
            #predict_beam으로 연결
            # 따로 max turn 제한은 없음. -> 추가
            prob_hyp = self.generator.predict(cxt, **params)
        probs = np.array([prob for prob, _ in prob_hyp])
        hyps = [hyp for _, hyp in prob_hyp]
        if wt_ranker > 0:
            # model.py 의 scorebase의 predict 로 연결
            # cxt = input and history?, hyps = answer
            # max turn 지정 가능
            scores_ranker = self.ranker.predict(cxt, hyps, max_cxt_turn=10)
